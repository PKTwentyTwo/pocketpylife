...
When ```pocketpylife.lifetree()``` is called, findgenera.py is used to determine which class of rules a given rulestring belongs to, then creates the corresponding type of ```Lifetree```.  There are four files that the class could be taken from:
 ...
 - [cpplifetree.py](../cplusplus/cpplifetree.py): The fastest version. It generates and compiles a C++ program at creation time, then uses a shared library to simulate OT rules up to 10x as fast as pure Python, depending on the OS, CPU, and compiler.
## Pattern
